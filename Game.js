...
BunnyDefender.Game.prototype = {
	...
	assignBunnyMovement: function(b) {
		...
		t=this.add.tween(b).to({x:bposition}, 3500, Phaser.Easing.Quadratic.InOut, true, bdelay); //to make the tween movement smooth
	...
	stopBunny: function(b) {
		...
		b.animations.play('Play', 24 ,true);
		this.assignBunnyMovement(b);
	...
	buildSpaceRocks: function() {
		...
		for(var i=0; i<this.totalSpacerocks; i++){
			...
			var scale = this.rnd.realInRange(0.3, 0.1);
			r.scale.x = scale;
			r.scale.y = scale;
	...
	respawnRock: function(r){
		...
		r.reset(this.rnd.integerInRange(0, this.world.width), this.rnd.integerInRange(-1500, 0));
		r.body.velocity.y = this.rnd.integerInRange(200,400);
		}
	},
	
	buildEmitter: function(){
		this.burst = this.add.emitter(0, 0, 100);
...
//once the entities overlap or collide respawn the rock
		this.respawnRock(r);
	},
	...
	makeGhost: function(b) {
		...
		bunnghost.body.velocity.y = -800;
		
	}, 	
	
    update : function() {
		this.physics.arcade.overlap(this.spacerockgroup, this.burst, this.burstCollision, null, this);
		// 'this' is the callback context to access the collision entities		
